...
function statusTone(status: string): "healthy" | "warning" | "neutral" | "error" {
  if (status === "Needs attention" || status === "Protected" || status === "Preview") {
    return "warning";
  }
...
function runStatus(run: RetentionRun): string {
  ...
  return run.dryRun ? "Preview" : "Completed";
}
      ...
      <div className="memory-workspace__detail-body">
        ...
        <DefinitionList
          items={[
            ...
            { label: "Status", value: memory.statusDetail },
            {
              label: "Related",
          ...
          return (
            <li key={`${item.entityId ?? "unknown"}-${index}`}>
              {memory ? (
                ...
                <strong>Memory record unavailable</strong>
              )}
              <span>
                {item.entityId ? `Memory ${item.entityId}` : "Unknown memory"}
                {item.outcome ? ` / ${displayType(item.outcome)}` : ""}
              </span>
            </li>
          );
        })}
  ...
  return (
    <>
      <DetailHeader
        ...
        title={run.dryRun ? "Retention preview" : "Retention run"}
        status={status}
      />
      <div className="memory-workspace__detail-body">
        <p>{run.summary}</p>
        ...
        <DefinitionList
          items={[
            ...
            { label: "Mode", value: run.dryRun ? "Preview only" : "Applied changes" },
            { label: "Requested by", value: run.actorId || "Service administrator" },
            { label: "Result", value: status },
          ]}
        />
        <ReportItems title="Flagged for review" items={run.flagged} memories={memories} onOpenMemory={onOpenMemory} />
        <ReportItems title="Deletion matches" items={run.deleted} memories={memories} onOpenMemory={onOpenMemory} />
        <ReportItems title="Skipped" items={run.skipped} memories={memories} onOpenMemory={onOpenMemory} />
      </div>
    </>
  );
}
...
export function MemoryWorkspace({
  ...
  const executeRetention = async () => {
    ...
    try {
      const report = await runRetention(agentId, false);
      await refreshData();
          ...
          {visibleMemories.map((memory) => (
            <li key={memory.id}>
              <RecordRow
                ...
                status={memory.state === "Retained" ? "Current" : memory.state}
                detail={memory.category ?? displayType(memory.entityType)}
                onSelect={() => {
              ...
              <section className="memory-workspace__section">
                <div className="memory-workspace__section-head">
                  <div>
                    ...
                    <p>Recent manual retention runs and previews for this agent.</p>
                  </div>
                </div>
            ...
            <div role="tabpanel" aria-label="Activity">
              <Grid className="memory-workspace__page-head memory-workspace__page-head--admin">
                <Column sm={4} md={8} lg={16} className="memory-workspace__page-copy">
                  ...
                  <h1>Activity</h1>
                  <p>Inspect the locally recorded history of manual retention previews and applied runs.</p>
                </Column>
              </Grid>
              <div className="memory-workspace__section-head">
                <div>
