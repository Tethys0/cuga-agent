...
function mapReport(report: RetentionReportResponse, dryRun = true): RetentionReport {
  return {
    runId: report.run_id,
    startedAt: report.started_at,
    completedAt: report.completed_at,
    dryRun: report.dry_run ?? dryRun,
...
export async function loadRetentionRuns(agentId: string): Promise<RetentionRun[]> {
  ...
  return (response.items ?? []).map((run) => ({
    ...mapReport(run.report, run.dry_run),
    runId: run.run_id,
...
export async function runRetention(agentId: string, dryRun: boolean): Promise<RetentionReport> {
  ...
  return mapReport(response, dryRun);
}
